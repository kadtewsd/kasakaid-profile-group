...
class MyConfiguration {
    ...
    fun check(env: Environment): List<String?> {
        ...
        if (env.activeProfiles.any { it == "development" }) {
            require(development1 == "include1。developmentプロファイルにより参照(ymlなのでUTF-8エンコードできます") { "間接的なインクルードができていません" }
            require(development2 == "include2。developmentプロファイルにより参照(ymlなのでUTF-8エンコードできます") { "間接的なインクルードができていません" }
